...
The Ruffier test measures pulse rate in three stages:
before physical exertion, immediately after it, and after a short rest.
The values are usually measured for 15 seconds and then converted to beats
per minute by multiplying by 4:

    S = 4 * (P1 + P2 + P3)

The further the result is from the ideal value of 200 beats per minute,
the worse the result is considered. Traditionally, tables are given for a
value divided by 10.
...
The result is estimated by age according to the table below:
...
the difference between the weak and satisfactory results is 5, and the
difference between the good and satisfactory results is 5.5.

Therefore, the ruffier_result(r_index, level) function receives the calculated
Ruffier index and the "unsatisfactory" level for the tested person's age, and
returns the result.

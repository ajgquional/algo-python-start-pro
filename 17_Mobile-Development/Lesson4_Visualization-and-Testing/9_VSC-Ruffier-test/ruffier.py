...
The sum of the three attempts at heart rate readings (before exertion,
right after exertion, and after a short break) ideally should be no more
than 200 beats per minute.

We suggest that the children measure their heart rate for 15 seconds,
and then get beats per minute by multiplying by 4:

    S = 4 * (P1 + P2 + P3)

The further the result is from the ideal 200 beats, the worse it is.
Traditionally, the table uses values divided by 10.

Ruffier index:

    RI = (S - 200) / 10

is evaluated according to age using the table:
...
The result "unsatisfactory" is 4 away from "weak" for all ages,
"weak" is 5 away from "satisfactory", and "good" is 5.5 away from
"satisfactory".

We therefore write a function `ruffier_result(r_index, level)` that
produces the calculated Ruffier index and the "unsatisfactory" level for
the tested age.
"""
